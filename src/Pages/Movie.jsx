...
export default function Movie() {
  ...
  async function checkRating() {
    ...
    const rated = await fench.get(`account/${user.id}/rated/movies`);

    const found = rated.data.results.find((m) => m.id === movie.id);
  ...
  async function ratingChanged(rate) {
    if (isRated) {
      await fench.delete(`movie/${movie.id}/rating`);
      setRatingNum(0);
      setIsRated(false);
